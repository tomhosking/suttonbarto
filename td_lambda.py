...
def GetAction(s):
    if np.random.random() > epsilon:
        return (policy[s], False)
    else:
        #print("xplore")
        return (actions[np.random.randint(0, len(actions))], True)
    ...
    while not terminated:
        ...
        for aprime in actions:
            ...
            if thisq > bestq or bestq is False:
                bestq = thisq
        delta = reward + gamma * bestq - Qvalue[(s,a)]
        eligibility[(s,a)] = eligibility[(s,a)] + 1

        for stemp in states:
            for atemp in actions:
                Qvalue[(stemp,atemp)] = Qvalue[(stemp,atemp)] + alpha * delta * eligibility[(stemp, atemp)]# Q learning
                # Qvalue[(s,a)] = Qvalue[(s,a)] + alpha * (reward + gamma * Qvalue[(sprime,aprime)] - Qvalue[(s,a)]) # sarsa
                if exploring:
                    eligibility[(stemp,atemp)] = 0
                else:
                    eligibility[(stemp,atemp)] = tracelambda * gamma * eligibility[(stemp,atemp)]
        s = sprime
        #a = aprime
        policy = UpdatePolicy(policy)
